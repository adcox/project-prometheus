 ...
 *
 * This arranges ships in diagonal rows
 */
public class Replicator_Mixed_6 extends Level {

    public Replicator_Mixed_6(){
        name = "Rogue Regiment";
    ...
    public ArrayList<Ship> initShips(){
        ...
        for(int row = 0; row < (numRows); row++){
            for(int collum = 0; collum < (numCollumns); collum++){
                ...
                ships.add(new Ship_Replicator((row + collum)%3 + 1, location, (stepX*3)/5, initX_Vel, firingProbibility, row, collum));
            }
        }
