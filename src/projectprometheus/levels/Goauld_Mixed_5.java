 ...
 *
 * This level has all types of Goa'uld ships in diagonals
 */
public class Goauld_Mixed_5 extends Level {
    
    public Goauld_Mixed_5(){
        name = "Horrible Hoard";
    ...
    public ArrayList<Ship> initShips(){
        ...
        for(int row = 0; row < (numRows); row++){
            for(int collum = 0; collum < (numCollumns); collum++){
                ...
                ships.add(new Ship_Goauld((row + collum)%3 + 1, location, (stepX*3)/5, initX_Vel, firingProbibility, row, collum));
            }
        }
